...
const AdminPortal: React.FC<AdminPanelProps> = ({ bookings, settings, onUpdateSettings, onLogout }) => {
  ...
  const handleResetShift = async () => {
    if (!window.confirm("Warning: This will mark all lockers as returned. Continue?")) return;
    const success = await cloudSync.checkoutShift();
    if (success) {
      alert("Shift Reset Done.");
      fetchRentals();
    }
  };
        ...
        <div className="flex flex-wrap justify-center bg-white/5 p-1.5 rounded-2xl mt-12 gap-1 border border-white/10">
            {['bookings', 'lockers', 'pricing', 'slots'].map(tab => (
              <button 
                ...
                className={`px-8 py-3 rounded-xl text-[10px] font-black uppercase transition-all ${activeTab === tab ? 'bg-white text-slate-900 shadow-xl' : 'text-white/50 hover:text-white'}`}
              >
                {tab}
              </button>
            ))}
        ...
        {activeTab === 'lockers' && (
          <div className="space-y-10">
             <div className="flex justify-between items-center">
                <h3 className="text-2xl font-black uppercase text-slate-900">Locker Management</h3>
                <div className="flex gap-2">
                   <button onClick={fetchRentals} className="bg-slate-100 px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest text-slate-400 hover:text-slate-900">
                      {isLoadingRentals ? <i className="fas fa-sync fa-spin"></i> : <i className="fas fa-sync"></i>} Refresh
                   </button>
                   <button onClick={handleResetShift} className="bg-red-50 text-red-600 px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest border border-red-100">Reset Shift</button>
                   <button onClick={() => navigate('/admin-lockers')} className="bg-blue-600 text-white px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest shadow-lg">Detailed View</button>
                </div>
             </div>

             <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 text-center">
                   <p className="text-[9px] font-black text-slate-400 uppercase mb-1">Active Issued</p>
                   <p className="text-2xl font-black">{stats.activeRentals}</p>
                </div>
                <div className="bg-emerald-50 p-6 rounded-3xl border border-emerald-100 text-center">
                   <p className="text-[9px] font-black text-emerald-600 uppercase mb-1">Security Cash</p>
                   <p className="text-2xl font-black text-emerald-700">₹{stats.securityHeld}</p>
                </div>
                <div className="bg-blue-50 p-6 rounded-3xl border border-blue-100 text-center">
                   <p className="text-[9px] font-black text-blue-400 uppercase mb-1">M-Busy</p>
                   <p className="text-2xl font-black text-blue-600">{stats.maleBusy}</p>
                </div>
                <div className="bg-pink-50 p-6 rounded-3xl border border-pink-100 text-center">
                   <p className="text-[9px] font-black text-pink-400 uppercase mb-1">F-Busy</p>
                ...
                <table className="w-full text-left">
                   <thead>
                      <tr className="text-[10px] font-black uppercase text-slate-400 border-b">
                         <th className="pb-4 px-2">Receipt</th>
                         <th className="pb-4 px-2">Guest</th>
                         <th className="pb-4 px-2">Numbers</th>
                         <th className="pb-4 px-2">Deposit</th>
                         <th className="pb-4 px-2">Status</th>
                      </tr>
                   </thead>
                   <tbody className="divide-y divide-slate-50">
                      {rentals.filter(r => r.status === 'issued').map(r => (
                        <tr key={r.receiptNo} className="text-xs font-bold text-slate-700">
                           ...
                           <td className="py-4 px-2">
                              {r.maleLockers.length > 0 && <span className="text-blue-500 mr-2">M:{r.maleLockers.join(',')}</span>}
                              {r.femaleLockers.length > 0 && <span className="text-pink-500">F:{r.femaleLockers.join(',')}</span>}
                           </td>
                           <td className="py-4 px-2">₹{r.securityDeposit}</td>
                           <td className="py-4 px-2">
                              <span className="bg-amber-100 text-amber-700 px-3 py-1 rounded-full text-[9px] font-black uppercase">ISSUED</span>
                           </td>
                        </tr>
                      ))}
                      {rentals.filter(r => r.status === 'issued').length === 0 && (
                        <tr><td colSpan={5} className="py-10 text-center text-slate-400 uppercase text-[10px]">No active lockers found.</td></tr>
                      )}
                   </tbody>
                </table>
             </div>
